...
| `is_international` based on the **customer's country currency**, not a static list | A Uruguayan paying in ARS counts as an international transaction. Implemented via a join with `dim_customers` and a country→currency mapping. Logic confirmed with instructor `@serasio`. |
